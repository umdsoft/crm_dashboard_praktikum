...
<template>
    <div v-if="do_render_html">
        ...
        <div class="fixed top-1/2 w-1/2 rounded-md left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white z-50 p-5">
            ...
            <h1>Qaysi mavzu normativi baxolanadi?</h1>

            <div class="mb-10 flex justify-between items-center">

                <select class="w-full px-5 py-2 focus:outline-none pr-12 bg-gray-100 rounded">
                    <option value="1">Normativ to‘liq topshirildi</option>
                    <option value="1">Normativ chala topshirildi</option>
                    <option value="0">Normativ topshirilmadi</option>
                </select>
            ...
            <table class="w-full text-sm rtl:text-right">
                <thead class="text-sm text-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-center">Normativni bajarish</th>
                ...
                <tbody class="text-base">
                    <tr v-for="(item, index) in props.students" :key="index" class="border-b hover:bg-gray-50 m-5">
                        ...
                        <td class="text-center">
                            <select v-model="sendData[index].isCheck"
                                class="w-full px-5 py-2 focus:outline-none pr-12 bg-gray-100 rounded">
                                <option value="1">Normativ to‘liq topshirildi</option>
                                <option value="1">Normativ chala topshirildi</option>
                                <option value="0">Normativ topshirilmadi</option>
                            </select>
                        ...
                        <td class="px-6 py-2">
                            <input v-model="sendData[index].reason"
                                ...
                                placeholder="Baxolanmaganlik sababi" />
                        </td>
                        <td class="text-center">
                            {{ dateFormat(new Date()) }}
                        </td>
